<template>
  <div class="d-flex flex-wrap align-start justify-space-evenly">
    <v-container style="max-width: 850px;">
      ...
      <v-container>
        <cite>
        <p class="text-h4">"L'unico modo per andare veloce è andare piano."</p>
      </cite>
      <p>- Robert C. Martin aka Uncle Bob (Sull'importanza della scrittura di codice pulito)</p>
      </v-container>
    </v-container>
    <v-container style="max-width: 850px;" class="mb-3">
      <v-card title="Form di contatto" variant="text" class="pt-3 pl-3 pr-3 pb-3" density="compact" elevation="3">
        <v-card-item>
          <v-form @submit.prevent role="form">
            ...
            <div class="align-left">
              <v-btn class="mt-2 mr-3" variant="text" @click="reset" role="button" aria-controls="Reset form">Resetta
                form</v-btn>
              <v-btn type="submit" class="mt-2" variant="text" role="button" aria-controls="Invia">Invia</v-btn>
            </div>
          </v-form>
        </v-card-item>
      </v-card>
...
const contacts = reactive([
  { id: 0, title: 'Gmail', subtitle: 'davidesabia22@gmail.com', color: 'red', img: 'mdi-google', reference: "mailto:davidesabia22@gmail.com?subject=Ciao Davide! Ti contatto dopo aver visitato il tuo sito web&body=Messaggio dello sviluppatore: Cercherò di risponderti quanto prima :)" },
  { id: 1, title: 'Github', subtitle: 'Dino-996', color: 'black', img: 'mdi-github', reference: 'https://github.com/Dino-996' },
  { id: 2, title: 'LinkedIn', subtitle: 'davidesabia', color: 'blue', img: 'mdi-linkedin', reference: 'https://it.linkedin.com/in/davidesabia' }
]);
...
const textBoxRules = [
  ...
  () => (textBox && textBox.value.length < 225) || 'Puoi inserire massimo 225 caratteri'
];
