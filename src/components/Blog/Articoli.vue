<template>
  <div class="mt-5">
    <v-text-field v-model="searchTerm" prepend-icon="mdi-magnify" label="Cerca per titolo" variant="underlined"
      :rules="titleRules" @input="searchByTitle"></v-text-field>
  </div>

  <div class="text-start">
    <v-radio-group v-model="sorted" inline>
      <v-radio label="Articoli meno recenti" value="cre" @click="sortByDateCre"></v-radio>
      <v-radio label="Articoli recenti" value="dec" @click="sortByDateDec"></v-radio>
    </v-radio-group>
  </div>

  <v-container v-if="displayedArticles.length > 0" class="d-flex flex-wrap flex-align-between">
      <v-card v-for="article in displayedArticles" :key="article.id" class="mt-3 ml-2 mb-3 mx-auto" max-width="344">
        ...
        <v-card-actions class="align-left">
          <v-btn class="text-subtitle-1" append-icon="mdi-arrow-u-right-bottom" @click="getArticle(article.id)">Leggi di pi&ugrave;</v-btn>
        </v-card-actions>
      </v-card>
  </v-container>
