          ...
          <v-card-item>
            <v-card-text>
              <div class="d-flex flex-wrap align-center justify-space-between">
                <div>
                  <v-avatar>
                    <v-img :src="article.avatar" :lazy-src="article.avatar" :alt="article.author">
                    </v-img>
                  </v-avatar>
                  {{ article.author }}
                </div>
                <div>
                  <v-icon size="small" class="mr-3">mdi-calendar</v-icon>{{ article.date }}
                </div>
              </div>
            </v-card-text>
          </v-card-item>
  ...
  return testoArticolo.value.replace(
    ...
    (match, p1, p2) => {
      switch (p1) {
        ...
        case 'Link':
          return (
            '<a style="font-size:1em; color: #1295d2;" href="' +
            p2 +
